export const SITE_CONFIG = {
  brandName: "Nexus",
  companyName: "Nexus Integrated Distribution System",
  version: "v1.0.1",
  currencySymbol: "Rp",
  numberLocale: "id-ID",
  dateFormat: "dd/MM/yyyy",
  timeFormat: "HH:mm",
  // Auto-logout after this many minutes without activity
  // (covers idle time, computer sleep/hibernate, and a closed browser).
  sessionTimeoutMinutes: 30,
  assets: {
    loginImage: "/images/company-logo.jpg",
  },
}

export type Language = "en" | "id"

export const SITE_CONTENT = {
  en: {
    // Login Page
    LOGIN_TITLE: "NIDS System",
    LOGIN_SUBTITLE: "Enter your credentials to access the Nexus",
    LOGIN_ERROR_PENDING:
      "Your account is pending approval from the administrator.",
    LOGIN_ERROR_INVALID: "Invalid username or password.",
    LABEL_USERNAME: "Username",
    PLACEHOLDER_USERNAME: "admin",
    LABEL_PASSWORD: "Password",
    PLACEHOLDER_PASSWORD: "••••••••",
    BUTTON_LOGIN: "Sign In",
    LINK_SIGNUP: "Sign Up",
    TEXT_NO_ACCOUNT: "Don't have an account?",
    COPYRIGHT: "© 2026 Nexus Integrated Distribution System",

    // Signup Page
    SIGNUP_TITLE: "Create Account",
    SIGNUP_SUBTITLE: "Join the Nexus, approval required.",
    SIGNUP_SUCCESS_TITLE: "Registration Successful!",
    SIGNUP_SUCCESS_MSG: "Your account is pending boss approval.",
    LABEL_FULL_NAME: "Full Name",
    LABEL_EMAIL: "Email Address",
    LABEL_PHONE: "Phone Number",
    PLACEHOLDER_PHONE: "+62...",
    TOOLTIP_LANG: "Switch Language",
    BUTTON_SIGNUP: "Request Access",
    BUTTON_SIGNUP_LOADING: "Creating Account...",
    LINK_LOGIN: "Sign In",
    LINK_BACK_LOGIN: "Back to Login",
    LINK_RETURN_LOGIN: "Return to Login",
    TEXT_HAS_ACCOUNT: "Already have an account?",

    // Navigation / Titles
    MENU_DASHBOARD: "Dashboard",
    MENU_COMPANIES: "Companies",
    MENU_PRODUCTS: "Products",
    MENU_QUOTES: "Quotes",
    MENU_ORDERS: "Orders",
    MENU_FUNDERS: "Funders",
    MENU_TRANSACTION: "Transaction",
    MENU_DEPOSIT: "Deposit",
    MENU_QUOTATION: "Quotation",
    MENU_SALES_ORDER: "Sales Order",
    MENU_DELIVERY_ORDER: "Delivery Order",
    MENU_VEHICLES: "Vehicles",
    MENU_INVOICE: "Invoice",
    MENU_PAYMENTS: "Payments",
    MENU_SETTINGS: "Settings",
    MENU_REPORTS_INVENTORY: "Inventory Report",
    MENU_REPORTS_PROFIT_LOSS: "Profit & Loss",

    // Quotation Module
    LABEL_QUOTATION_NUMBER: "Quotation Number",
    LABEL_QUOTATION_DATE: "Quotation Date",
    LABEL_EXPIRY_DATE: "Expiry Date",
    LABEL_VALIDITY_DAYS: "Validity Days",
    LABEL_MIN_ORDER: "Min. Order",
    LABEL_STATUS: "Status",
    LABEL_CONTENT: "Quotation Content",
    LABEL_NOTE: "Note",
    LABEL_TERMS: "Terms and Conditions",
    LABEL_CLOSING: "Closing Remarks",
    LABEL_SHRINKAGE_TOLERANCE: "Shrinkage Tolerance",
    LABEL_SHRINKAGE_IN_PRICE: "",
    LABEL_TRANSPORT_COST: "Transport Cost",
    LABEL_DELIVERY_ADDRESS: "Delivery Address",
    LABEL_MARGIN: "Margin",
    LABEL_TOTAL_PRICE: "Total Price",
    LABEL_PRICE_PER_L: "Price per L",
    LABEL_DELIVERY_PER_L: "Delivery per L",
    LABEL_BANK_ACCOUNTS: "Bank Accounts",
    LABEL_DISCOUNTS: "Discounts",
    LABEL_DISCOUNT_TERM1: "Term 1",
    LABEL_DISCOUNT_TERM2: "Term 2",
    LABEL_DISCOUNT_VALUE: "Value (%)",
    LABEL_DISCOUNT_TERMS: "Discount / Price Terms",
    LABEL_TERM_OF_PAYMENT: "Term of Payment",
    LABEL_TAX_VAT: "Enable Tax (VAT)",
    LABEL_TAX_RATE: "Tax Rate (%)",
    LABEL_TAXES: "Taxes",
    LABEL_SUBTOTAL: "Subtotal",
    LABEL_DELIVERY_TOTAL: "Delivery Total",
    LABEL_GRAND_TOTAL: "Grand Total",
    LABEL_TOTAL_DEPOSITS: "Total Deposits",
    LABEL_TOTAL_QUOTATIONS: "Total Quotations",
    LABEL_DRAFT_QUOTATIONS: "Draft Quotations",
    LABEL_SENT_QUOTATIONS: "Sent Quotations",
    LABEL_ALMOST_EXPIRED: "Almost Expired",
    LABEL_STATUS_DRAFT: "Draft",
    LABEL_STATUS_SENT: "Sent",
    LABEL_STATUS_OVERDUE: "Overdue",
    LABEL_STATUS_PAID: "Paid",
    LABEL_TOTAL_SOS: "Total Sales Orders",
    LABEL_TOTAL_INVOICES: "Total Invoices",
    LABEL_TOTAL_PAYMENTS: "Total Payments",
    LABEL_TOTAL_AMOUNT: "Total Amount",
    LABEL_TOTAL_VOLUME: "Total Volume",
    LABEL_TOTAL_CUSTOMERS: "Total Customers",
    LABEL_TOTAL_SUPPLIERS: "Total Suppliers",
    LABEL_TOTAL_TRANSPORTERS: "Total Transporters",
    LABEL_TOTAL_VEHICLES: "Total Vehicles",
    LABEL_TOTAL_USERS: "Total Users",
    LABEL_ACTIVE_VEHICLES: "Active Vehicles",
    LABEL_TOTAL_DOS: "Total Delivery Orders",
    LABEL_TOTAL_VALUE: "Total Value",
    LABEL_ACCEPTED: "Accepted",
    LABEL_COMPLETED: "Completed",
    LABEL_DELIVERED: "Delivered",
    LABEL_VERIFIED: "Verified",
    LABEL_IN_TRANSIT: "In Transit",
    LABEL_LOGISTICS: "Logistics",
    LABEL_WAREHOUSE_ID: "Warehouse ID",
    LABEL_AVG_COST: "Avg Cost",
    LABEL_INVENTORY_VALUE: "Inventory Value",
    LABEL_ACTIVE_WAREHOUSES: "Active Warehouses",
    LABEL_PENDING_AMOUNT: "Pending Amount",
    LABEL_PAID_AMOUNT: "Paid Amount",
    LABEL_FROM_DATE: "From Date",
    LABEL_TO_DATE: "To Date",
    LABEL_OPTIONAL: "Optional",
    LABEL_AUTO_GENERATED: "[Auto Generated]",

    BUTTON_NEW_QUOTATION: "New Quotation",
    BUTTON_EDIT_QUOTATION: "Edit Quotation",
    BUTTON_ADD_DISCOUNT: "Add Discount",
    BUTTON_SAVE_QUOTATION: "Save Quotation",

    MSG_QUOTATION_SAVED: "Quotation %data% saved successfully.",
    MSG_QUOTATION_DELETED: "Quotation %data% removed.",
    MSG_QUOTATION_STATUS_UPDATED: "Quotation status %data% updated.",
    MSG_DELETE_CONFIRM: "Are you sure you want to delete this?",
    MSG_EXPIRY_INFO: "Expiry is calculated based on Quotation Date.",
    MSG_VALIDATION_ERROR: "Validation Error",
    MSG_COMPANY_REQUIRED: "Company must be selected.",
    MSG_PRODUCT_REQUIRED: "Product/SKU must be selected.",

    // Deposit Module
    LABEL_DEPOSIT_NUMBER: "Deposit Number",
    LABEL_DEPOSIT_DATE: "Deposit Date",
    LABEL_PAYMENT_METHOD: "Payment Method",
    LABEL_AMOUNT: "Amount",
    LABEL_REMAINING: "Rem",
    BUTTON_NEW_DEPOSIT: "New Deposit",
    BUTTON_SAVE_DEPOSIT: "Save Deposit",
    MSG_DEPOSIT_SAVED: "Deposit %data% saved successfully.",
    MSG_DEPOSIT_DELETED: "Deposit %data% removed.",
    MSG_DEPOSIT_STATUS_UPDATED: "Deposit status %data% updated.",
    MSG_DEPOSIT_INVENTORY_IN_USE:
      "Cannot modify deposit %data%. Inventory from this supplier/product has been used in delivery orders.",
    MSG_DEPOSIT_DELETE_BLOCKED:
      "This deposit cannot be deleted because its inventory is already in use.",

    // Sales Order Module
    LABEL_SO_NUMBER: "SO Number",
    LABEL_PO_NUMBER: "PO Number",
    LABEL_PO_DATE: "PO Date",
    PLACEHOLDER_ENTER_PO_NUMBER: "Customer PO number",
    LABEL_SO_DATE: "SO Date",
    LABEL_DELIVERY_DATE: "Delivery Date",
    LABEL_QUANTITY: "Quantity",
    LABEL_UNIT_PRICE: "Unit Price",
    BUTTON_NEW_SO: "New Sales Order",
    BUTTON_NEW_INVOICE: "New Invoice",
    BUTTON_RECORD_PAYMENT: "Record Payment",
    BUTTON_SAVE_SO: "Save Sales Order",
    MSG_SO_SAVED: "Sales Order %data% saved successfully.",
    MSG_SO_DELETED: "Sales Order %data% removed.",
    MSG_SO_STATUS_UPDATED: "Sales Order status %data% updated.",

    // SO AI Import
    BUTTON_IMPORT_DOC: "Import from Document",
    IMPORT_DOC_TITLE: "Import from Document",
    IMPORT_DOC_DESCRIPTION:
      "Upload a Purchase Order document (PDF or image). AI will read it and fill the new Sales Order form for you.",
    IMPORT_DOC_DROP_HINT: "Drag & drop files here, or",
    IMPORT_DOC_SELECT_FILES: "Choose Files",
    IMPORT_DOC_LIMITS: "PDF, PNG, JPEG, or WebP. Max 4 files, 10 MB each.",
    IMPORT_DOC_ANALYZE: "Analyze",
    IMPORT_DOC_ANALYZING: "Analyzing...",
    IMPORT_DOC_USE_DATA: "Use This Data",
    IMPORT_DOC_WARNINGS: "AI Warnings",
    IMPORT_DOC_SJ_NUMBER: "Surat Jalan No.",
    IMPORT_DOC_TIMINGS: "Processing time",
    IMPORT_DOC_TIMINGS_DETAIL:
      "Upload %upload% · AI analysis %ai% · Verification %verify% · Total %total%",
    IMPORT_DOC_VERIFY_FAILED:
      "Arithmetic verification could not be run — totals were not audited.",
    IMPORT_DOC_TAXES: "Taxes",
    IMPORT_DOC_TAXABLE_YES: "Yes — delivery fee is included in PPN",
    IMPORT_DOC_TAXABLE_NO: "No — delivery fee is not taxed",
    IMPORT_DOC_TAXABLE_UNKNOWN: "Not stated in document",
    IMPORT_DOC_MATCHED_HINT:
      "Matched company/product will be pre-selected. You can change them in the form.",
    IMPORT_DOC_FAILED_TITLE: "AI Extraction Failed",
    IMPORT_DOC_FAILED_DESC: "Could not extract data from the document(s).",
    IMPORT_DOC_INVALID_TYPE_TITLE: "Unsupported File",
    IMPORT_DOC_INVALID_TYPE_DESC: "Only PDF, PNG, JPEG, or WebP files are allowed.",
    IMPORT_DOC_TOO_LARGE_TITLE: "File Too Large",
    IMPORT_DOC_TOO_LARGE_DESC: "Maximum file size is 10 MB.",

    // Delivery Order Module
    LABEL_DO_NUMBER: "DO Number",
    LABEL_DO_DATE: "DO Date",
    LABEL_SHIPMENT_DATE: "Shipment Date",
    LABEL_INVOICE_NUMBER: "Invoice Number",
    LABEL_ISSUE_DATE: "Issue Date",
    LABEL_DUE_DATE: "Due Date",
    LABEL_PAYMENT_NUMBER: "Payment Number",
    LABEL_PAYMENT_DATE: "Payment Date",
    LABEL_REFERENCE_NUMBER: "Reference Number",
    LABEL_LINK_TO_PO: "Link to PO",
    LABEL_LINK_TO_DO: "Link to DO",
    BUTTON_EDIT_INVOICE: "Edit Invoice",
    PLACEHOLDER_SELECT_DO: "Search DO number...",
    LABEL_FILL_SO_LATER: "Fill SO later",
    LABEL_SO_PENDING: "Pending SO",
    LABEL_SO_PENDING_HINT:
      "This DO cannot be invoiced until its SO number is filled.",
    MSG_SO_REQUIRED_FOR_INVOICE:
      "This Delivery Order has no Sales Order. Fill the SO number first on the Delivery Order page.",
    BILLING_NOTE_EXCEEDS_SENT:
      "Received quantity exceeds sent quantity. Billed quantity is capped at sent quantity.",
    BILLING_NOTE_EXCEEDS_TOLERANCE:
      "Loss exceeds shrinkage tolerance. Billing received quantity plus the tolerance allowance.",
    BILLING_NOTE_WITHIN_TOLERANCE:
      "Loss is within shrinkage tolerance. Billing original sent quantity.",
    LABEL_PRODUCT: "Product",
    LABEL_TOTAL: "Total",
    LABEL_TAX_INCLUDED: "Include Tax (PPN)",
    LABEL_TAX_AUTO_CALC: "Calculate tax automatically",
    LABEL_DELIVERY_TAXABLE: "Include Delivery Fee in Tax (PPN)",
    LABEL_ENABLE_NOTE: "Enable Note",
    LABEL_INTERNAL_NOTE: "Internal Note",
    LABEL_DATES: "Dates",
    LABEL_AMOUNTS: "Amounts",
    LABEL_PAID: "Paid",
    LABEL_VERIFY: "Verify",
    LABEL_REJECT: "Reject",
    LABEL_DRIVER_NAME: "Driver Name",
    LABEL_VEHICLE_NUMBER: "Vehicle Number",
    LABEL_VEHICLE: "Vehicle",
    LABEL_LICENSE_NUMBER: "License Number",
    LABEL_VEHICLE_TYPE: "Vehicle Type",
    LABEL_TOTAL_CAPACITY: "Total Capacity",
    LABEL_CAPACITY: "Capacity",
    LABEL_COMPARTMENTS: "Compartments",
    LABEL_SEALS: "Seals",
    LABEL_NUMBER_OF_SEALS: "Number of Seals",
    LABEL_SEAL_NUMBER: "Seal Number",
    LABEL_COMPARTMENT_NUMBER: "Komp #",
    BUTTON_NEW_DO: "New Delivery Order",
    BUTTON_SAVE_DO: "Save Delivery Order",
    BUTTON_ADD_COMPARTMENT: "Add Compartment",
    MSG_DO_SAVED: "Delivery Order %data% saved successfully.",
    MSG_DO_DELETED: "Delivery Order %data% removed.",
    MSG_AUTO_DEPOSIT_CREATED:
      "Auto-created deposit %data% to cover supplier stock (price is 0 — update it on the Deposits page).",
    MSG_DO_STATUS_UPDATED: "Delivery Order status %data% updated.",
    LABEL_QTY_SHIPPED: "Qty Shipped",
    LABEL_QTY_RECEIVED: "Qty Received",
    LABEL_RECEIVED_DATE: "Date Received",
    MSG_CONFIRM_DELIVERY: "Confirm Delivery",
    MSG_SO_REQUIRED: "Sales Order must be selected.",
    LABEL_SO_INFORMATION: "SO Information",
    LABEL_CUSTOMER_FILTER: "Customer Company",
    LABEL_SO_REQUIRED: "Sales Order",
    LABEL_SO_TOTAL_QTY: "SO Total Quantity",
    LABEL_QTY_DELIVERED: "Delivered",
    LABEL_DELIVERY_DETAILS: "Delivery Details",
    LABEL_PRICE_DISCOUNT: "Price Discount",
    LABEL_DELIVERY_FEE: "Delivery Fee",
    LABEL_CALCULATION_DETAILS: "Calculation Details",
    LABEL_DATE_SENT: "Date Sent",
    LABEL_DATE_DELIVERED: "Date Delivered",
    LABEL_TRANSPORTER: "Transporter",
    LABEL_DRIVER_PHONE: "Driver Phone",
    PLACEHOLDER_SELECT_SO: "Select Sales Order...",
    PLACEHOLDER_SELECT_COMPANY: "Select Company...",
    PLACEHOLDER_SELECT_TRANSPORTER: "Select Transporter...",
    PLACEHOLDER_SELECT_SUPPLIER: "Select Supplier...",
    PLACEHOLDER_SELECT_DRIVER: "Select or enter driver...",
    PLACEHOLDER_ENTER_DRIVER_PHONE: "Enter driver phone",
    PLACEHOLDER_SELECT_VEHICLE: "Select a vehicle...",
    PLACEHOLDER_SELECT_PRODUCT: "Select product...",
    PLACEHOLDER_FROM_SO: "From SO...",
    MSG_NO_ACCEPTED_SO: "No accepted SO found.",
    MSG_NO_VEHICLES: "No vehicles found.",
    MSG_SELECT_VEHICLE: "Select a vehicle to see seals",
    MSG_TYPE_DRIVER_HINT: "Type driver name and press '+' to add.",
    LABEL_ADDITIONAL_NOTES: "Additional Notes",
    LABEL_SUPPLIER_ORIGIN: "Supplier (Origin)",
    LABEL_STOCK: "Stock",
    LABEL_PREVIEW_DO: "Preview Delivery Order",
    LABEL_CLICK_PREVIEW: "Click to preview",
    LABEL_PREVIOUS: "Previous",
    LABEL_NEXT: "Next",
    LABEL_PAGE: "Page",
    LABEL_CLOSE: "Close",
    LABEL_DOWNLOAD_PDF: "Download PDF",
    LABEL_SEND_EMAIL: "Send to Customer",
    MSG_CONFIRM_EMAIL_DO: "Are you sure you want to send this DO to",
    LABEL_SORT: "Sort",
    LABEL_APPLY: "Apply",

    // Invoice Module (multi-DO / SO source)
    LABEL_INVOICE_SOURCE: "Invoice Source",
    LABEL_SOURCE_BY_DO: "Delivery Order",
    LABEL_SOURCE_BY_SO: "Sales Order",
    LABEL_ADD_DO: "Add DO",
    LABEL_SELECTED_DOS: "Selected Delivery Orders",
    LABEL_DO_LIST: "DO List",
    LABEL_TOTAL_QUANTITY: "Total Quantity",
    LABEL_REVIEW_SUMMARY: "Review Summary",
    LABEL_INVOICE_SUMMARY: "Invoice Summary",
    LABEL_DELIVERY_PROGRESS: "Delivery Progress",
    MSG_SELECT_AT_LEAST_ONE_DO: "Select at least one Delivery Order.",
    MSG_SELECT_SO: "Select a Sales Order.",
    MSG_DO_SAME_SO_REQUIRED:
      "All Delivery Orders must share the same Sales Order.",
    MSG_DO_SAME_COMPANY_REQUIRED:
      "All Delivery Orders must belong to the same company.",
    MSG_INCOMPLETE_DELIVERY:
      "Delivery is incomplete: %delivered% of %total% L delivered.",
    PLACEHOLDER_SELECT_SO_INVOICE: "Search SO number...",

    // Permission Messages
    MSG_ACCESS_DENIED: "Access Denied",
    MSG_NO_PERMISSION: "You do not have permission to view this page.",

    MENU_USERS: "Users",
    MENU_LOGOUT: "Logout",
    MENU_GROUP_MASTER: "Master Data",
    MENU_GROUP_REPORTS: "Reports",
    MENU_GROUP_SYSTEM: "System",
    MENU_SHOWCASE: "UI Showcase",

    TITLE_COMPANIES: "Companies",
    TITLE_PRODUCTS: "Products",
    TITLE_FUNDERS: "Funders",

    TITLE_ADD_COMPANY: "Add Company",
    TITLE_EDIT_COMPANY: "Edit Company",
    TITLE_ADD_PRODUCT: "Add Product",
    TITLE_EDIT_PRODUCT: "Edit Product",
    TITLE_ADD_FUNDER: "Add Funder",
    TITLE_EDIT_FUNDER: "Edit Funder",
    TITLE_DELETE: "Confirm Delete",

    // Funder Module
    LABEL_FUNDERS: "Funders",
    LABEL_FUNDERS_SECTION: "Funders",
    LABEL_ADD_FUNDER: "Add Funder",
    LABEL_FUNDERS_LIST_EMPTY: "No funders added yet",
    LABEL_FUNDER_NAME: "Funder",
    LABEL_FUNDER_AMOUNT: "Amount",
    LABEL_COMPANY_CONTRIBUTION: "Company Covers",
    LABEL_TOTAL_FUNDERS: "Total Funders",
    LABEL_MANAGE_FUNDERS: "Manage Funders",

    DASHBOARD_TITLE: "Executive Dashboard",
    LABEL_THIS_WEEK: "this week",
    LABEL_NEW_QUOTATIONS: "New Quotations",
    LABEL_EXPIRING_QUOTATIONS: "Expiring Soon",
    LABEL_PENDING_SO: "Pending Sales Orders",
    LABEL_UNDELIVERED_DO: "Undelivered DOs",
    LABEL_OVERDUE_INVOICES: "Overdue Invoices",
    LABEL_DUE_SOON_INVOICES: "Due Soon",
    LABEL_PENDING_PAYMENTS: "Pending Payments",
    LABEL_PENDING_DO: "Pending PO",
    LABEL_ACTIVE_COMPANIES: "Active",

    // Labels
    LABEL_NAME: "Name",
    LABEL_COMPANY_NAME: "Company Name",
    LABEL_NICKNAME: "Nickname",
    LABEL_PRODUCT_NAME: "Product Name",
    LABEL_SKU: "SKU",
    LABEL_TYPE: "Type",
    LABEL_CATEGORY: "Category",
    LABEL_UNIT: "Unit",
    LABEL_PRICE: "Price",
    LABEL_BASE_PRICE: "Base Price",
    LABEL_ACTIONS: "Actions",
    LABEL_CONTACT_PERSON: "Contact Person",
    LABEL_DESCRIPTION: "Description",
    LABEL_CITY: "City",
    LABEL_ADDRESS: "Address",
    LABEL_ADDRESS_LABEL: "Address Label",
    LABEL_NPWP: "NPWP (Tax ID)",
    LABEL_ID_NUMBER: "ID Number",
    LABEL_OTHER_INFO: "Other Info",
    LABEL_TYPE_CUSTOMER: "Customer",
    LABEL_TYPE_SUPPLIER: "Supplier",
    LABEL_TYPE_TRANSPORTER: "Transporter",
    LABEL_COMPANY_CONTACT_INFO: "Company Information",
    LABEL_DIGITAL_SIGNATURE: "Digital Signature",
    LABEL_CC_EMAILS: "CC Emails",
    PLACEHOLDER_CC_EMAILS: "email1@example.com, email2@example.com",
    TOOLTIP_CC_EMAILS:
      "These emails will be CC'd on all company-related documents",
    LABEL_BCC_EMAILS: "BCC Emails",
    PLACEHOLDER_BCC_EMAILS: "email1@example.com, email2@example.com",
    TOOLTIP_BCC_EMAILS:
      "These emails will be BCC'd on all company-related documents",

    // General UI
    BUTTON_SAVE: "Save",
    BUTTON_CANCEL: "Cancel",
    BUTTON_BACK: "Back",
    BUTTON_ADD: "Add New",
    BUTTON_EDIT: "Edit",
    BUTTON_DELETE: "Delete",
    BUTTON_REFRESH: "Refresh",
    PLACEHOLDER_SEARCH: "Search...",
    PLACEHOLDER_SELECT_ADDRESS: "Select an address...",
    PLACEHOLDER_SELECT_TERM: "Select term...",
    PLACEHOLDER_ENTER_ADDRESS: "Enter delivery address...",
    PLACEHOLDER_SELECT_QUOTATION: "Select a quotation...",
    PLACEHOLDER_TOP: "e.g. COD, Net 30",
    PLACEHOLDER_EDITOR: "Start typing...",
    LOADING: "Loading...",
    NO_DATA: "No data found.",
    LABEL_ALL: "All",
    LABEL_SORT_BY: "Sort By",
    LABEL_THEN_BY: "",
    LABEL_ASCENDING: "A to Z (Ascending)",
    LABEL_DESCENDING: "Z to A (Descending)",
    BUTTON_ADD_LEVEL: "Add Level",
    BUTTON_DELETE_LEVEL: "Delete Level",
    TITLE_SORT_SETTINGS: "Sort Configuration",

    // Status & Notifications
    MSG_STATUS_UPDATED: "Status %data% Updated",
    MSG_UPDATE_SUCCESS: "Update %data% Successful",
    MSG_DELETE_SUCCESS: "Data %data% Successfully Deleted",
    MSG_UPDATE_FAILED: "Update %data% Failed",
    MSG_SAVE_SUCCESS: "Saved %data% Successfully",
    MSG_SAVE_FAILED: "Save %data% Failed",
    MSG_DATA_FETCH_FAILED: "Data Fetch Failed",
    MSG_SESSION_EXPIRED: "Session Expired",
    MSG_RELOGIN: "Your session has expired. Please sign in again to continue.",
    MSG_LOGGING_OUT: "You are being logged out...",
    MSG_SUCCESS_SAVE_DESC: "Successfully saved %entity% for %company%.",
    MSG_SUCCESS_SAVE_DESC_NO_COMPANY: "Successfully saved %entity%.",
    MSG_SUCCESS_UPDATE_DESC: "Successfully updated %entity% for %company%.",
    MSG_SUCCESS_UPDATE_DESC_NO_COMPANY: "Successfully updated %entity%.",
    MSG_SUCCESS_DELETE_DESC: "Successfully deleted %entity% for %company%.",
    MSG_SUCCESS_DELETE_DESC_NO_COMPANY: "Successfully deleted %entity%.",
    MSG_SUCCESS_STATUS_DESC:
      "Successfully changed status to %status% for %company%.",
    MSG_SUCCESS_STATUS_DESC_NO_COMPANY:
      "Successfully changed status to %status%.",
    MSG_PRINT_SUCCESS: "PDF generated successfully.",
    MSG_PRINT_SUCCESS_DESC:
      "Successfully downloaded and prepared document %data% for printing.",
    MSG_EMAIL_SENT_SUCCESS: "Email sent to customer successfully.",
    MSG_EMAIL_SENT_SUCCESS_DESC: "Document %data% status updated to Sent.",
    ERR_EMAIL_FAILED: "Failed to send email",
    LABEL_IS_ACTIVE: "Active",
    LABEL_IS_INACTIVE: "Inactive",
    MSG_PRODUCT_STATUS: "Product is now",
    MSG_COMPANY_STATUS: "Company is now",

    LABEL_LANGUAGE: "Language",
    LABEL_THEME: "Theme",
    LABEL_LIGHT: "Light",
    LABEL_DARK: "Dark",
    LABEL_SYSTEM: "System",
    LABEL_ACTIVE_FUNDERS: "Active Funders",
    // User Management
    TITLE_USER_MGMT: "User Management",
    TITLE_MANAGE_USER: "Manage User",
    LABEL_USER: "User",
    LABEL_PHONE_SHORT: "Phone",
    LABEL_ROLE: "Role",
    LABEL_APPROVED: "Approved",
    LABEL_PENDING: "Inactive",
    LABEL_ACTIVE: "Active",
    LABEL_DEACTIVATED: "Deactivated",
    LABEL_MODULE: "Module",
    LABEL_GRANULAR_PERMS: "Granular Permissions",
    LABEL_ACC_APPROVAL: "Account Approval",
    DESC_ACC_APPROVAL: "Approve this user to give them access to the system.",
    BUTTON_APPROVE: "Approve User",
    BUTTON_REVOKE: "Revoke Approval",
    BUTTON_MANAGE: "Manage",

    // Validation Errors
    ERROR_INVALID_EMAIL: "Please enter a valid email address.",
    ERROR_PASSWORD_SHORT: "Password must be at least 6 characters.",
    ERROR_EMAIL_EXISTS: "This email address is already in use.",
    ERROR_USERNAME_EXISTS:
      "This username is already taken. Please try another one.",
    ERROR_PHONE_EXISTS: "This phone number is already registered.",
    ERROR_INFO_EXISTS: "An account with this information already exists.",
    ERROR_REG_RESTRICTED:
      "Registration is currently restricted. Please contact an admin.",
    ERROR_SUBMIT_FAILED:
      "Unable to submit request. Please check your information.",
    ERROR_UNEXPECTED: "An unexpected error occurred. Please try again.",
    ERROR_TIMEOUT:
      "Request timed out. Please check your connection and try again.",
    ERROR_VERIFY_STATUS: "Unable to verify account status.",
    ERROR_NICKNAME_EXISTS:
      'The nickname "%nickname%" is already used by company "%company%". Please use a different nickname.',

    // Additional Labels
    LABEL_NO_AUTH: "No Auth",
    LABEL_LAST_LOGIN: "Last Login",
    LABEL_USERNAME_FIELD: "Username",
    PLACEHOLDER_LOGIN_FALLBACK: "admin or email@example.com",

    // Reset Password / Forgot Password
    TITLE_RESET_PWD: "Reset Password",
    DESC_RESET_PWD: "Securely update your Nexus password.",
    TITLE_SETUP_ACC: "Account Setup",
    DESC_SETUP_ACC: "Complete your Nexus account activation.",
    LABEL_NEW_PASSWORD: "New Password",
    LABEL_CONFIRM_PASSWORD: "Confirm New Password",
    BUTTON_SET_NEW_PWD: "Set New Password",
    MSG_VERIFYING_TOKEN: "Verifying security token...",
    ERROR_EXPIRED_LINK:
      "This security link has expired. Please contact your administrator to revoke and re-approve your account access.",
    ERROR_INVALID_LINK:
      "Invalid security link. Please use the link provided in your email.",
    MSG_SECURITY_UPDATED: "Security Updated",
    MSG_RESET_LINK_SENT: "Security Link Dispatched",
    DESC_SECURITY_REDIRECT:
      "Your password has been set successfully. You are being redirected to the login page to sign in with your new credentials.",
    LINK_FORGOT_PWD: "Forgot password?",
    TITLE_FORGOT_PWD: "Reset Password",
    DESC_FORGOT_PWD:
      "Enter your email address and we'll send you a link to reset your password.",
    MSG_RESET_SENT:
      "If an account exists with this email, you will receive a password reset link shortly.",
    BUTTON_SEND_LINK: "Send Link",
    ERROR_PASSWORD_MISMATCH: "Passwords do not match. Please try again.",

    // Settings Page
    SETTINGS_TITLE: "Global Settings",
    SETTINGS_SUBTITLE:
      "Manage enterprise identities, dynamic tax rates, and core operational configurations.",
    SETTINGS_TAB_COMPANY: "Company Profile",
    SETTINGS_TAB_BANKS: "Bank Accounts",
    SETTINGS_TAB_PARAMETERS: "System Parameters",

    // Settings Form Company Profile
    SETTINGS_SEC_COMPANY: "Enterprise Identity Specifications",
    SETTINGS_LABEL_NPWP: "NPWP (Tax Number)",
    SETTINGS_LABEL_LOGO_URL: "Brand Logo URL",
    SETTINGS_LABEL_HEADER_URL: "PDF Header Image URL",
    SETTINGS_LABEL_NPWP_ADDRESS: "NPWP Address",
    SETTINGS_LABEL_SHIPPING_ADDRESS: "Operational / Shipping Office Address",
    SETTINGS_PLACEHOLDER_NPWP_ADDRESS:
      "Official address registered in NPWP profile",
    SETTINGS_PLACEHOLDER_SHIPPING_ADDRESS: "Operational headquarters address",
    SETTINGS_BUTTON_SAVE_COMPANY: "Save Specifications",

    // Settings Bank Accounts
    SETTINGS_SEC_BANKS: "Relational Settlement Bank Accounts",
    SETTINGS_ADD_BANK_ACC: "Add Bank Account",
    SETTINGS_EDIT_BANK_ACC: "Modify Bank Account",
    SETTINGS_LABEL_BANK_NAME: "Bank Name",
    SETTINGS_LABEL_ACC_NUM: "Account Number",
    SETTINGS_LABEL_ACC_HOLDER: "Account Holder Name",
    SETTINGS_LABEL_BRANCH: "Branch Name",
    SETTINGS_PLACEHOLDER_BANK_NAME:
      "e.g. Bank Mandiri, BCA, Bank Rakyat Indonesia",
    SETTINGS_PLACEHOLDER_ACC_NUM: "e.g. 111231002319",
    SETTINGS_PLACEHOLDER_ACC_HOLDER: "e.g. PT Anugerah Buana Sriwijaya",
    SETTINGS_PLACEHOLDER_BRANCH: "e.g. Letkol Iskandar",

    // Settings Parameters
    SETTINGS_SEC_PARAMS: "Dynamic Operational Parameters",
    SETTINGS_BUTTON_SAVE_PARAMS: "Save Parameters",
    SETTINGS_ADD_PARAM: "Add Parameter",
    SETTINGS_LABEL_PARAM_KEY: "Parameter Key",
    SETTINGS_LABEL_PARAM_VALUE: "Parameter Value",
    SETTINGS_LABEL_PARAM_LABEL: "Parameter Label",
    SETTINGS_LABEL_PARAM_DESC: "Parameter Description",

    // Settings Email Routing
    SETTINGS_TAB_EMAILS: "Email Routing",
    SETTINGS_SEC_EMAILS: "Automated Email Routing (CC)",
    SETTINGS_BUTTON_SAVE_EMAILS: "Save Routing Rules",

    // Verification Portal
    VERIFY_TITLE: "Document Verification",
    VERIFY_SUBTITLE: "Securely verify the authenticity of Nexus documents.",
    VERIFY_LABEL_QUOTATION_NUMBER: "Quotation Number",
    VERIFY_PLACEHOLDER_QUOTATION_NUMBER: "e.g. QTN/2026/001",
    VERIFY_BUTTON_VERIFY: "Verify Document",
    VERIFY_BUTTON_DOWNLOADING: "Downloading...",
    VERIFY_SUCCESS_TITLE: "Authenticity Confirmed",
    VERIFY_SUCCESS_MSG:
      "This is an authentic document issued by PT Anugerah Buana Sriwijaya.",
    VERIFY_MISMATCH_TITLE: "Authenticity Not Confirmed",
    VERIFY_MISMATCH_MSG:
      "This document's data may have been modified since this QR code was generated.",
    VERIFY_ERROR_TITLE: "Verification Failed",
    VERIFY_ERROR_MSG:
      "The quotation number does not match this record. Please check and try again.",
    VERIFY_ERROR_RATE_LIMIT:
      "Too many failed attempts. Please try again in a few minutes.",
    VERIFY_DOC_DETAILS: "Document Details",
    VERIFY_LABEL_COMPANY: "Company",
    VERIFY_LABEL_DATE: "Issue Date",
    VERIFY_LABEL_TOTAL: "Grand Total",
    VERIFY_BUTTON_DOWNLOAD_OFFICIAL: "Download Official PDF",

    VERIFY_STATUS_HASH_MATCH: "Hash Matched",
    VERIFY_STATUS_HASH_MISMATCH: "Hash Mismatch",
    VERIFY_STATUS_HASH_MISMATCH_DESC:
      "Document data may have been modified since this QR code was generated.",
    VERIFY_STATUS_HASH_NOT_AVAILABLE:
      "Hash verification not available for this document",
    VERIFY_STATUS_HASH_VERIFIED: "Automatically verified via QR hash",
    VERIFY_AUTO_VERIFYING: "Verifying document via QR code...",
    VERIFY_LABEL_INVOICE_NUMBER: "Invoice Number",
    VERIFY_PLACEHOLDER_INVOICE_NUMBER: "e.g. INV/2026/001",
  },
  id: {
    // Login Page
    LOGIN_TITLE: "Sistem NIDS",
    LOGIN_SUBTITLE: "Masukkan kredensial Anda untuk mengakses Nexus",
    LOGIN_ERROR_PENDING:
      "Akun Anda sedang menunggu persetujuan dari administrator.",
    LOGIN_ERROR_INVALID: "Nama pengguna atau kata sandi salah.",
    LABEL_USERNAME: "Nama Pengguna",
    PLACEHOLDER_USERNAME: "admin",
    LABEL_PASSWORD: "Kata Sandi",
    PLACEHOLDER_PASSWORD: "••••••••",
    BUTTON_LOGIN: "Masuk",
    LINK_SIGNUP: "Daftar",
    TEXT_NO_ACCOUNT: "Belum punya akun?",
    COPYRIGHT: "© 2026 Nexus Integrated Distribution System",

    // Signup Page
    SIGNUP_TITLE: "Buat Akun",
    SIGNUP_SUBTITLE: "Masuk ke Nexus. Perlu persetujuan bos.",
    SIGNUP_SUCCESS_TITLE: "Pendaftaran Berhasil!",
    SIGNUP_SUCCESS_MSG: "Akun Anda sedang menunggu persetujuan bos.",
    LABEL_FULL_NAME: "Nama Lengkap",
    LABEL_EMAIL: "Alamat Email",
    LABEL_PHONE: "Nomor Telepon",
    PLACEHOLDER_PHONE: "081xxx...",
    TOOLTIP_LANG: "Ganti Bahasa",
    BUTTON_SIGNUP: "Ajukan Akses",
    BUTTON_SIGNUP_LOADING: "Membuat Akun...",
    LINK_LOGIN: "Login",
    LINK_BACK_LOGIN: "Kembali ke Login",
    LINK_RETURN_LOGIN: "Kembali ke Login",
    TEXT_HAS_ACCOUNT: "Sudah punya akun?",

    // Navigation / Titles
    MENU_DASHBOARD: "Dashboard",
    MENU_COMPANIES: "Perusahaan",
    MENU_PRODUCTS: "Produk",
    MENU_QUOTES: "Penawaran",
    MENU_ORDERS: "Pesanan",
    MENU_FUNDERS: "Funder",
    MENU_TRANSACTION: "Transaksi",
    MENU_DEPOSIT: "Deposit",
    MENU_QUOTATION: "Penawaran",
    MENU_SALES_ORDER: "Sales Order",
    MENU_DELIVERY_ORDER: "Surat Jalan",
    MENU_VEHICLES: "Kendaraan",
    MENU_INVOICE: "Invoice",
    MENU_PAYMENTS: "Pembayaran",
    MENU_SETTINGS: "Pengaturan",
    MENU_REPORTS_INVENTORY: "Inventory Report",
    MENU_REPORTS_PROFIT_LOSS: "Profit & Loss",

    // Quotation Module
    LABEL_QUOTATION_NUMBER: "Nomor Penawaran",
    LABEL_QUOTATION_DATE: "Tanggal Penawaran",
    LABEL_EXPIRY_DATE: "Tanggal Expired",
    LABEL_VALIDITY_DAYS: "Masa Berlaku (Hari)",
    LABEL_DAYS: "Day(s)",
    LABEL_MIN_ORDER: "Min. Pesanan",
    LABEL_STATUS: "Status",
    LABEL_CONTENT: "Isi Penawaran",
    LABEL_NOTE: "Catatan",
    LABEL_TERMS: "Syarat dan Ketentuan",
    LABEL_CLOSING: "Kata Penutup",
    LABEL_SHRINKAGE_TOLERANCE: "Toleransi Susut",
    LABEL_SHRINKAGE_IN_PRICE: "",
    LABEL_TRANSPORT_COST: "Biaya Transportasi",
    LABEL_DELIVERY_ADDRESS: "Alamat Pengiriman",
    LABEL_MARGIN: "Margin",
    LABEL_TOTAL_PRICE: "Total Harga",
    LABEL_PRICE_PER_L: "Harga per L",
    LABEL_DELIVERY_PER_L: "Pengiriman per L",
    LABEL_BANK_ACCOUNTS: "Rekening Bank",
    LABEL_DISCOUNTS: "Diskon",
    LABEL_DISCOUNT_TERM1: "Syarat 1",
    LABEL_DISCOUNT_TERM2: "Syarat 2",
    LABEL_DISCOUNT_VALUE: "Nilai (%)",
    LABEL_DISCOUNT_TERMS: "Ketentuan Harga / Diskon",

    LABEL_TERM_OF_PAYMENT: "Termin Pembayaran",
    LABEL_TAX_VAT: "Aktifkan Pajak (PPN)",
    LABEL_TAX_RATE: "Tarif Pajak (%)",
    LABEL_SUBTOTAL: "Subtotal",
    LABEL_DELIVERY_TOTAL: "Total Pengiriman",
    LABEL_GRAND_TOTAL: "Total",
    LABEL_TAXES: "Pajak",
    LABEL_TOTAL_DEPOSITS: "Total Deposit",
    LABEL_TOTAL_QUOTATIONS: "Total Penawaran",
    LABEL_DRAFT_QUOTATIONS: "Penawaran Draft",
    LABEL_SENT_QUOTATIONS: "Penawaran Terkirim",
    LABEL_ALMOST_EXPIRED: "Hampir Expired",
    LABEL_STATUS_DRAFT: "Draft",
    LABEL_STATUS_SENT: "Terkirim",
    LABEL_STATUS_OVERDUE: "Jatuh Tempo",
    LABEL_STATUS_PAID: "Lunas",
    LABEL_TOTAL_SOS: "Total SO",
    LABEL_TOTAL_INVOICES: "Total Invoice",
    LABEL_TOTAL_PAYMENTS: "Total Pembayaran",
    LABEL_TOTAL_AMOUNT: "Total Jumlah",
    LABEL_TOTAL_VOLUME: "Total Volume",
    LABEL_TOTAL_CUSTOMERS: "Pelanggan",
    LABEL_TOTAL_SUPPLIERS: "Supplier",
    LABEL_TOTAL_TRANSPORTERS: "Transporters",
    LABEL_TOTAL_VEHICLES: "Total Kendaraan",
    LABEL_TOTAL_USERS: "Total User",
    LABEL_ACTIVE_VEHICLES: "Kendaraan Aktif",
    LABEL_TOTAL_DOS: "Total Surat Jalan",
    LABEL_TOTAL_VALUE: "Total Nilai",
    LABEL_ACCEPTED: "Diterima",
    LABEL_COMPLETED: "Selesai",
    LABEL_DELIVERED: "Terkirim",
    LABEL_VERIFIED: "Terverifikasi",
    LABEL_IN_TRANSIT: "Dalam Perjalanan",
    LABEL_LOGISTICS: "Logistik",
    LABEL_WAREHOUSE_ID: "ID Gudang",
    LABEL_AVG_COST: "Harga Rata-rata",
    LABEL_INVENTORY_VALUE: "Nilai Inventory",
    LABEL_ACTIVE_WAREHOUSES: "Gudang Aktif",
    LABEL_PENDING_AMOUNT: "Jumlah Tertunda",
    LABEL_PAID_AMOUNT: "Jumlah Dibayar",
    LABEL_FROM_DATE: "Dari Tanggal",
    LABEL_TO_DATE: "Sampai Tanggal",
    LABEL_OPTIONAL: "Opsional",
    LABEL_AUTO_GENERATED: "[Otomatis]",
    PLACEHOLDER_SELECT_QUOTATION: "Pilih penawaran...",
    PLACEHOLDER_TOP: "Contoh: COD, Net 30",

    BUTTON_NEW_QUOTATION: "Penawaran Baru",
    BUTTON_EDIT_QUOTATION: "Edit Penawaran",
    BUTTON_ADD_DISCOUNT: "Tambah Diskon",
    BUTTON_SAVE_QUOTATION: "Simpan Penawaran",

    MSG_QUOTATION_SAVED: "Penawaran %data% berhasil disimpan.",
    MSG_QUOTATION_DELETED: "Penawaran %data% berhasil dihapus.",
    MSG_QUOTATION_STATUS_UPDATED: "Status penawaran %data% diperbarui.",
    MSG_DELETE_CONFIRM: "Apakah Anda yakin ingin menghapus ini?",
    MSG_EXPIRY_INFO: "Expired dihitung berdasarkan Tanggal Penawaran.",
    MSG_VALIDATION_ERROR: "Validasi Gagal",
    MSG_COMPANY_REQUIRED: "Perusahaan harus dipilih.",
    MSG_PRODUCT_REQUIRED: "Produk/SKU harus dipilih.",

    // Deposit Module
    LABEL_DEPOSIT_NUMBER: "Nomor Deposit",
    LABEL_DEPOSIT_DATE: "Tanggal Deposit",
    LABEL_PAYMENT_METHOD: "Metode Pembayaran",
    LABEL_AMOUNT: "Jumlah",
    LABEL_REMAINING: "Sisa",
    BUTTON_NEW_DEPOSIT: "Deposit Baru",
    BUTTON_SAVE_DEPOSIT: "Simpan Deposit",
    MSG_DEPOSIT_SAVED: "Deposit %data% berhasil disimpan.",
    MSG_DEPOSIT_DELETED: "Deposit %data% berhasil dihapus.",
    MSG_DEPOSIT_STATUS_UPDATED: "Status Deposit %data% diperbarui.",
    MSG_DEPOSIT_INVENTORY_IN_USE:
      "Tidak dapat mengubah Deposit %data%. Stok dari supplier/produk ini sudah digunakan dalam surat jalan.",
    MSG_DEPOSIT_DELETE_BLOCKED:
      "Deposit ini tidak dapat dihapus karena stoknya sudah digunakan.",

    // Sales Order Module
    LABEL_SO_NUMBER: "Nomor SO",
    LABEL_PO_NUMBER: "Nomor PO",
    LABEL_PO_DATE: "Tanggal PO",
    PLACEHOLDER_ENTER_PO_NUMBER: "Nomor PO pelanggan",
    LABEL_SO_DATE: "Tanggal SO",
    LABEL_DELIVERY_DATE: "Tanggal Pengiriman",
    LABEL_QUANTITY: "Kuantitas",
    LABEL_UNIT_PRICE: "Harga Satuan",
    BUTTON_NEW_SO: "SO Baru",
    BUTTON_NEW_INVOICE: "Invoice Baru",
    BUTTON_RECORD_PAYMENT: "Catat Pembayaran",
    BUTTON_SAVE_SO: "Simpan SO",
    MSG_SO_SAVED: "Sales Order %data% berhasil disimpan.",
    MSG_SO_DELETED: "Sales Order %data% berhasil dihapus.",
    MSG_SO_STATUS_UPDATED: "Status Sales Order %data% diperbarui.",

    // SO AI Import
    BUTTON_IMPORT_DOC: "Import dari Dokumen",
    IMPORT_DOC_TITLE: "Import dari Dokumen",
    IMPORT_DOC_DESCRIPTION:
      "Upload dokumen Purchase Order (PDF atau gambar). AI akan membacanya dan mengisi form SO baru untuk Anda.",
    IMPORT_DOC_DROP_HINT: "Tarik & lepas file di sini, atau",
    IMPORT_DOC_SELECT_FILES: "Pilih File",
    IMPORT_DOC_LIMITS: "PDF, PNG, JPEG, atau WebP. Maks 4 file, 10 MB per file.",
    IMPORT_DOC_ANALYZE: "Analisis",
    IMPORT_DOC_ANALYZING: "Menganalisis...",
    IMPORT_DOC_USE_DATA: "Gunakan Data Ini",
    IMPORT_DOC_WARNINGS: "Peringatan AI",
    IMPORT_DOC_SJ_NUMBER: "No. Surat Jalan",
    IMPORT_DOC_TIMINGS: "Waktu proses",
    IMPORT_DOC_TIMINGS_DETAIL:
      "Upload %upload% · Analisis AI %ai% · Verifikasi %verify% · Total %total%",
    IMPORT_DOC_VERIFY_FAILED:
      "Verifikasi perhitungan tidak dapat dijalankan — total tidak diaudit.",
    IMPORT_DOC_TAXES: "Pajak",
    IMPORT_DOC_TAXABLE_YES: "Ya — ongkos kirim termasuk PPN",
    IMPORT_DOC_TAXABLE_NO: "Tidak — ongkos kirim tidak kena pajak",
    IMPORT_DOC_TAXABLE_UNKNOWN: "Tidak disebutkan di dokumen",
    IMPORT_DOC_MATCHED_HINT:
      "Perusahaan/produk yang cocok akan dipilih otomatis. Anda dapat mengubahnya di form.",
    IMPORT_DOC_FAILED_TITLE: "Extract AI Gagal",
    IMPORT_DOC_FAILED_DESC: "Tidak dapat extract data dari dokumen.",
    IMPORT_DOC_INVALID_TYPE_TITLE: "File Tidak Didukung",
    IMPORT_DOC_INVALID_TYPE_DESC: "Hanya file PDF, PNG, JPEG, atau WebP yang diizinkan.",
    IMPORT_DOC_TOO_LARGE_TITLE: "File Terlalu Besar",
    IMPORT_DOC_TOO_LARGE_DESC: "Ukuran file maksimal 10 MB.",

    // Delivery Order Module
    LABEL_DO_NUMBER: "Nomor DO",
    LABEL_DO_DATE: "Tanggal DO",
    LABEL_SHIPMENT_DATE: "Tanggal Pengiriman",
    LABEL_INVOICE_NUMBER: "Nomor Invoice",
    LABEL_ISSUE_DATE: "Tanggal Terbit",
    LABEL_DUE_DATE: "Jatuh Tempo",
    LABEL_PAYMENT_NUMBER: "Nomor Pembayaran",
    LABEL_PAYMENT_DATE: "Tanggal Pembayaran",
    LABEL_REFERENCE_NUMBER: "Nomor Referensi",
    LABEL_LINK_TO_PO: "Link ke PO",
    LABEL_LINK_TO_DO: "Link ke DO",
    BUTTON_EDIT_INVOICE: "Edit Invoice",
    PLACEHOLDER_SELECT_DO: "Cari nomor DO...",
    LABEL_FILL_SO_LATER: "Isi SO nanti",
    LABEL_SO_PENDING: "SO Tertunda",
    LABEL_SO_PENDING_HINT:
      "DO tidak dapat dibuatkan faktur sebelum nomor SO diisi.",
    MSG_SO_REQUIRED_FOR_INVOICE:
      "DO ini belum memiliki Sales Order. Isi nomor SO terlebih dahulu di halaman Delivery Order.",
    BILLING_NOTE_EXCEEDS_SENT:
      "Jumlah diterima melebihi jumlah dikirim. Jumlah ditagih dibatasi pada jumlah dikirim.",
    BILLING_NOTE_EXCEEDS_TOLERANCE:
      "Susut melebihi toleransi. Menagih jumlah diterima ditambah toleransi.",
    BILLING_NOTE_WITHIN_TOLERANCE:
      "Susut dalam batas toleransi. Menagih jumlah awal yang dikirim.",
    LABEL_PRODUCT: "Produk",
    LABEL_TOTAL: "Total",
    LABEL_TAX_INCLUDED: "Termasuk Pajak (PPN)",
    LABEL_TAX_AUTO_CALC: "Hitung pajak otomatis",
    LABEL_DELIVERY_TAXABLE: "Masukkan Biaya Pengiriman dalam Pajak (PPN)",
    LABEL_ENABLE_NOTE: "Aktifkan Catatan",
    LABEL_INTERNAL_NOTE: "Catatan Internal",
    LABEL_DATES: "Tanggal",
    LABEL_AMOUNTS: "Jumlah",
    LABEL_PAID: "Dibayar",
    LABEL_VERIFY: "Verifikasi",
    LABEL_REJECT: "Tolak",
    LABEL_DRIVER_NAME: "Nama Sopir",
    LABEL_VEHICLE_NUMBER: "Nomor Kendaraan",
    LABEL_VEHICLE: "Kendaraan",
    LABEL_LICENSE_NUMBER: "Nomor Polisi",
    LABEL_VEHICLE_TYPE: "Tipe Kendaraan",
    LABEL_TOTAL_CAPACITY: "Total Kapasitas",
    LABEL_CAPACITY: "Kapasitas",
    LABEL_COMPARTMENTS: "Kompartemen",
    LABEL_SEALS: "Segel",
    LABEL_NUMBER_OF_SEALS: "Jumlah Segel",
    LABEL_SEAL_NUMBER: "Nomor Segel",
    LABEL_COMPARTMENT_NUMBER: "Komp #",
    BUTTON_NEW_DO: "DO Baru",
    BUTTON_SAVE_DO: "Simpan DO",
    BUTTON_ADD_COMPARTMENT: "Tambah Kompartemen",
    MSG_DO_SAVED: "Surat Jalan %data% berhasil disimpan.",
    MSG_DO_DELETED: "Surat Jalan %data% berhasil dihapus.",
    MSG_AUTO_DEPOSIT_CREATED:
      "Deposit %data% otomatis dibuat untuk menutup stok supplier (harga 0 — perbarui di halaman Deposit).",
    MSG_DO_STATUS_UPDATED: "Status Surat Jalan %data% diperbarui.",
    LABEL_QTY_SHIPPED: "Qty Dikirim",
    LABEL_QTY_RECEIVED: "Qty Diterima",
    LABEL_RECEIVED_DATE: "Tanggal Diterima",
    MSG_CONFIRM_DELIVERY: "Konfirmasi Pengiriman",
    MSG_SO_REQUIRED: "Pesanan Penjualan harus dipilih.",
    LABEL_SO_INFORMATION: "Informasi SO",
    LABEL_CUSTOMER_FILTER: "Perusahaan Pelanggan",
    LABEL_SO_REQUIRED: "Pesanan Penjualan",
    LABEL_SO_TOTAL_QTY: "Total Kuantitas SO",
    LABEL_QTY_DELIVERED: "Dikirim",
    LABEL_DELIVERY_DETAILS: "Detail Pengiriman",
    LABEL_PRICE_DISCOUNT: "Diskon Harga",
    LABEL_DELIVERY_FEE: "Biaya Pengiriman",
    LABEL_CALCULATION_DETAILS: "Rincian Perhitungan",
    LABEL_DATE_SENT: "Tanggal Dikirim",
    LABEL_DATE_DELIVERED: "Tanggal Diterima",
    LABEL_TRANSPORTER: "Transporter",
    LABEL_DRIVER_PHONE: "Telepon Sopir",
    PLACEHOLDER_SELECT_SO: "Pilih Pesanan Penjualan...",
    PLACEHOLDER_SELECT_COMPANY: "Pilih Perusahaan...",
    PLACEHOLDER_SELECT_TRANSPORTER: "Pilih Transporter...",
    PLACEHOLDER_SELECT_SUPPLIER: "Pilih Supplier...",
    PLACEHOLDER_SELECT_DRIVER: "Pilih atau ketik sopir...",
    PLACEHOLDER_ENTER_DRIVER_PHONE: "Masukkan telepon sopir",
    PLACEHOLDER_SELECT_VEHICLE: "Pilih kendaraan...",
    PLACEHOLDER_SELECT_PRODUCT: "Pilih produk...",
    PLACEHOLDER_FROM_SO: "Dari SO...",
    MSG_NO_ACCEPTED_SO: "SO yang diterima tidak ditemukan.",
    MSG_NO_VEHICLES: "Kendaraan tidak ditemukan.",
    MSG_SELECT_VEHICLE: "Pilih kendaraan untuk melihat segel",
    MSG_TYPE_DRIVER_HINT: "Ketik nama sopir dan tekan '+' untuk menambahkan.",
    LABEL_ADDITIONAL_NOTES: "Catatan Tambahan",
    LABEL_SUPPLIER_ORIGIN: "Supplier (Asal)",
    LABEL_STOCK: "Stok",
    LABEL_PREVIEW_DO: "Preview Surat Jalan",
    LABEL_CLICK_PREVIEW: "Klik untuk preview",
    LABEL_PREVIOUS: "Sebelumnya",
    LABEL_NEXT: "Berikutnya",
    LABEL_PAGE: "Halaman",
    LABEL_CLOSE: "Tutup",
    LABEL_DOWNLOAD_PDF: "Download PDF",
    LABEL_SEND_EMAIL: "Kirim ke Pelanggan",
    MSG_CONFIRM_EMAIL_DO: "Apakah Anda yakin ingin mengirim DO ini ke",
    LABEL_SORT: "Urutkan",
    LABEL_APPLY: "Terapkan",

    // Invoice Module (multi-DO / SO source)
    LABEL_INVOICE_SOURCE: "Sumber Invoice",
    LABEL_SOURCE_BY_DO: "Surat Jalan",
    LABEL_SOURCE_BY_SO: "Sales Order",
    LABEL_ADD_DO: "Tambah DO",
    LABEL_SELECTED_DOS: "DO Terpilih",
    LABEL_DO_LIST: "Daftar DO",
    LABEL_TOTAL_QUANTITY: "Total Kuantitas",
    LABEL_REVIEW_SUMMARY: "Ringkasan Tinjauan",
    LABEL_INVOICE_SUMMARY: "Ringkasan Invoice",
    LABEL_DELIVERY_PROGRESS: "Progres Pengiriman",
    MSG_SELECT_AT_LEAST_ONE_DO: "Pilih minimal satu DO.",
    MSG_SELECT_SO: "Pilih Sales Order.",
    MSG_DO_SAME_SO_REQUIRED: "Semua DO harus memiliki SO yang sama.",
    MSG_DO_SAME_COMPANY_REQUIRED:
      "Semua DO harus milik perusahaan yang sama.",
    MSG_INCOMPLETE_DELIVERY:
      "Pengiriman belum lengkap: %delivered% dari %total% L terkirim.",
    PLACEHOLDER_SELECT_SO_INVOICE: "Cari nomor SO...",

    // Permission Messages
    MSG_ACCESS_DENIED: "Akses Ditolak",
    MSG_NO_PERMISSION: "Anda tidak memiliki izin untuk melihat halaman ini.",

    MENU_USERS: "User",

    MENU_LOGOUT: "Logout",
    MENU_GROUP_MASTER: "Data Master",
    MENU_GROUP_REPORTS: "Laporan",
    MENU_GROUP_SYSTEM: "Sistem",
    MENU_SHOWCASE: "Showcase UI",

    TITLE_COMPANIES: "Perusahaan",
    TITLE_PRODUCTS: "Produk",
    TITLE_FUNDERS: "Funders",
    LABEL_ACTIVE_FUNDERS: "Funders Aktif",
    TITLE_ADD_COMPANY: "Tambah Perusahaan",
    TITLE_EDIT_COMPANY: "Edit Perusahaan",
    TITLE_ADD_PRODUCT: "Tambah Produk",
    TITLE_EDIT_PRODUCT: "Edit Produk",
    TITLE_ADD_FUNDER: "Tambah Funder",
    TITLE_EDIT_FUNDER: "Edit Funder",
    TITLE_DELETE: "Konfirmasi Delete",

    // Funder Module
    LABEL_FUNDERS: "Funder",
    LABEL_FUNDERS_SECTION: "Funder",
    LABEL_ADD_FUNDER: "Tambah Funder",
    LABEL_FUNDERS_LIST_EMPTY: "Belum ada funder",
    LABEL_FUNDER_NAME: "Nama Funder",
    LABEL_FUNDER_AMOUNT: "Jumlah",
    LABEL_COMPANY_CONTRIBUTION: "Ditanggung Perusahaan",
    LABEL_TOTAL_FUNDERS: "Total Funder",
    LABEL_MANAGE_FUNDERS: "Kelola Funder",

    DASHBOARD_TITLE: "Dashboard Eksekutif",
    LABEL_THIS_WEEK: "minggu ini",
    LABEL_NEW_QUOTATIONS: "Penawaran Baru",
    LABEL_EXPIRING_QUOTATIONS: "Akan Expired",
    LABEL_PENDING_SO: "SO Tertunda",
    LABEL_UNDELIVERED_DO: "SJ Belum Terkirim",
    LABEL_OVERDUE_INVOICES: "Invoice Jatuh Tempo",
    LABEL_DUE_SOON_INVOICES: "Segera Jatuh Tempo",
    LABEL_PENDING_PAYMENTS: "Pembayaran Tertunda",
    LABEL_PENDING_DO: "PO Tertunda",
    LABEL_ACTIVE_COMPANIES: "Aktif",

    // Labels
    LABEL_NAME: "Nama",
    LABEL_COMPANY_NAME: "Nama Perusahaan",
    LABEL_NICKNAME: "Kode Panggilan",
    LABEL_PRODUCT_NAME: "Nama Produk",
    LABEL_SKU: "SKU",
    LABEL_TYPE: "Tipe",
    LABEL_CATEGORY: "Kategori",
    LABEL_UNIT: "Satuan",
    LABEL_PRICE: "Harga",
    LABEL_BASE_PRICE: "Harga Dasar",
    LABEL_ACTIONS: "Aksi",
    LABEL_CONTACT_PERSON: "Kontak Person",
    LABEL_DESCRIPTION: "Deskripsi",
    LABEL_CITY: "Kota",
    LABEL_ADDRESS: "Alamat",
    LABEL_ADDRESS_LABEL: "Label Alamat",
    LABEL_NPWP: "NPWP",
    LABEL_ID_NUMBER: "Nomor KTP",
    LABEL_OTHER_INFO: "Info Lainnya",
    LABEL_TYPE_CUSTOMER: "Pelanggan",
    LABEL_TYPE_SUPPLIER: "Supplier",
    LABEL_TYPE_TRANSPORTER: "Transporter",
    LABEL_COMPANY_CONTACT_INFO: "Informasi Perusahaan",
    LABEL_DIGITAL_SIGNATURE: "Tanda Tangan Digital",
    LABEL_CC_EMAILS: "Email CC",
    PLACEHOLDER_CC_EMAILS: "email1@contoh.com, email2@contoh.com",
    TOOLTIP_CC_EMAILS:
      "Email ini akan di-CC pada semua dokumen terkait perusahaan",
    LABEL_BCC_EMAILS: "Email BCC",
    PLACEHOLDER_BCC_EMAILS: "email1@contoh.com, email2@contoh.com",
    TOOLTIP_BCC_EMAILS:
      "Email ini akan di-BCC pada semua dokumen terkait perusahaan",

    // General UI
    BUTTON_SAVE: "Simpan",
    BUTTON_CANCEL: "Batal",
    BUTTON_BACK: "Kembali",
    BUTTON_ADD: "Tambah Baru",
    BUTTON_EDIT: "Edit",
    BUTTON_DELETE: "Hapus",
    BUTTON_REFRESH: "Refresh",
    PLACEHOLDER_SEARCH: "Cari...",
    PLACEHOLDER_SELECT_ADDRESS: "Pilih alamat...",
    PLACEHOLDER_SELECT_TERM: "Pilih termin...",
    PLACEHOLDER_ENTER_ADDRESS: "Masukkan alamat pengiriman...",
    PLACEHOLDER_EDITOR: "Mulai mengetik...",
    LOADING: "Memuat...",
    NO_DATA: "Data tidak ditemukan.",
    LABEL_ALL: "Semua",
    LABEL_SORT_BY: "Urutkan",
    LABEL_THEN_BY: "",
    LABEL_ASCENDING: "A ke Z (Meningkat)",
    LABEL_DESCENDING: "Z ke A (Menurun)",
    BUTTON_ADD_LEVEL: "Tambah Level",
    BUTTON_DELETE_LEVEL: "Hapus Level",
    TITLE_SORT_SETTINGS: "Konfigurasi Pengurutan",

    // Status & Notifications
    MSG_STATUS_UPDATED: "Status %data% Diperbarui",
    MSG_UPDATE_SUCCESS: "Edit Data %data% Berhasil",
    MSG_DELETE_SUCCESS: "Data %data% berhasil dihapus.",
    MSG_UPDATE_FAILED: "Edit Data %data% Gagal",
    MSG_SAVE_SUCCESS: "Simpan Data %data% Berhasil",
    MSG_SAVE_FAILED: "Simpan Data %data% Gagal",
    MSG_DATA_FETCH_FAILED: "Gagal Mengambil Data",
    MSG_SESSION_EXPIRED: "Sesi Berakhir",
    MSG_RELOGIN:
      "Sesi Anda telah berakhir. Silakan login kembali untuk melanjutkan.",
    MSG_LOGGING_OUT: "Sedang Logout...",
    MSG_SUCCESS_SAVE_DESC: "Berhasil menyimpan %entity% untuk %company%.",
    MSG_SUCCESS_SAVE_DESC_NO_COMPANY: "Berhasil menyimpan %entity%.",
    MSG_SUCCESS_UPDATE_DESC: "Berhasil memperbarui %entity% untuk %company%.",
    MSG_SUCCESS_UPDATE_DESC_NO_COMPANY: "Berhasil memperbarui %entity%.",
    MSG_SUCCESS_DELETE_DESC: "Berhasil menghapus %entity% untuk %company%.",
    MSG_SUCCESS_DELETE_DESC_NO_COMPANY: "Berhasil menghapus %entity%.",
    MSG_SUCCESS_STATUS_DESC:
      "Berhasil mengubah status menjadi %status% untuk %company%.",
    MSG_SUCCESS_STATUS_DESC_NO_COMPANY:
      "Berhasil mengubah status menjadi %status%.",
    MSG_PRINT_SUCCESS: "PDF berhasil dibuat.",
    MSG_PRINT_SUCCESS_DESC:
      "Berhasil didownload dan disiapkan dokumen %data% untuk dicetak.",
    MSG_EMAIL_SENT_SUCCESS: "Email berhasil dikirim ke pelanggan.",
    MSG_EMAIL_SENT_SUCCESS_DESC:
      "Status dokumen %data% diperbarui menjadi Terkirim.",
    ERR_EMAIL_FAILED: "Gagal mengirim email",
    LABEL_IS_ACTIVE: "Aktif",
    LABEL_IS_INACTIVE: "Nonaktif",
    MSG_PRODUCT_STATUS: "Produk sekarang",
    MSG_COMPANY_STATUS: "Perusahaan sekarang",

    LABEL_LANGUAGE: "Bahasa",
    LABEL_THEME: "Tema",
    LABEL_LIGHT: "Terang",
    LABEL_DARK: "Gelap",
    LABEL_SYSTEM: "Sistem",

    // User Management
    TITLE_USER_MGMT: "Manajemen User",
    TITLE_MANAGE_USER: "Kelola User",
    LABEL_USER: "User",
    LABEL_PHONE_SHORT: "Telepon",
    LABEL_ROLE: "Peran",
    LABEL_APPROVED: "Disetujui",
    LABEL_PENDING: "Non-Aktif",
    LABEL_ACTIVE: "Aktif",
    LABEL_DEACTIVATED: "Dinonaktifkan",
    LABEL_MODULE: "Modul",
    LABEL_GRANULAR_PERMS: "Izin Granular",
    LABEL_ACC_APPROVAL: "Persetujuan Akun",
    DESC_ACC_APPROVAL:
      "Setujui pengguna ini untuk memberi mereka akses ke sistem.",
    BUTTON_APPROVE: "Setujui Pengguna",
    BUTTON_REVOKE: "Cabut Persetujuan",
    BUTTON_MANAGE: "Kelola",

    // Validation Errors
    ERROR_INVALID_EMAIL: "Silakan masukkan alamat email yang valid.",
    ERROR_PASSWORD_SHORT: "Password harus minimal 6 karakter.",
    ERROR_EMAIL_EXISTS: "Alamat email ini sudah digunakan.",
    ERROR_USERNAME_EXISTS:
      "Username ini sudah diambil. Silakan coba yang lain.",
    ERROR_PHONE_EXISTS: "Nomor telepon ini sudah terdaftar.",
    ERROR_INFO_EXISTS: "Akun dengan informasi ini sudah ada.",
    ERROR_REG_RESTRICTED:
      "Pendaftaran saat ini dibatasi. Silakan hubungi admin.",
    ERROR_SUBMIT_FAILED:
      "Tidak dapat mengajukan permintaan. Silakan periksa informasi Anda.",
    ERROR_UNEXPECTED:
      "Terjadi kesalahan yang tidak terduga. Silakan coba lagi.",
    ERROR_TIMEOUT:
      "Request timeout. Silakan periksa koneksi Anda dan coba lagi.",
    ERROR_VERIFY_STATUS: "Tidak dapat memverifikasi status akun.",
    ERROR_NICKNAME_EXISTS:
      'Nama panggilan "%nickname%" sudah digunakan oleh perusahaan "%company%". Silakan gunakan nama panggilan yang berbeda.',

    // Additional Labels
    LABEL_NO_AUTH: "Belum Ada Akun",
    LABEL_LAST_LOGIN: "Login Terakhir",
    LABEL_USERNAME_FIELD: "Username",
    PLACEHOLDER_LOGIN_FALLBACK: "admin atau email@example.com",

    // Reset Password / Forgot Password
    TITLE_RESET_PWD: "Ganti Password",
    DESC_RESET_PWD: "Perbarui password Nexus Anda dengan aman.",
    TITLE_SETUP_ACC: "Pengaturan Akun",
    DESC_SETUP_ACC: "Selesaikan aktivasi akun Nexus Anda.",
    LABEL_NEW_PASSWORD: "Password Baru",
    LABEL_CONFIRM_PASSWORD: "Konfirmasi Password Baru",
    BUTTON_SET_NEW_PWD: "Simpan Password Baru",
    MSG_VERIFYING_TOKEN: "Memverifikasi token keamanan...",
    ERROR_EXPIRED_LINK:
      "Link keamanan ini telah expired. Silakan hubungi administrator Anda.",
    ERROR_INVALID_LINK:
      "Link keamanan tidak valid. Silakan gunakan link yang disediakan di email Anda.",
    MSG_SECURITY_UPDATED: "Keamanan Diperbarui",
    MSG_RESET_LINK_SENT: "Link Keamanan Dikirim",
    DESC_SECURITY_REDIRECT:
      "Password Anda telah berhasil disimpan. Anda sedang dialihkan ke halaman login.",
    LINK_FORGOT_PWD: "Lupa password?",
    TITLE_FORGOT_PWD: "Ganti Password",
    DESC_FORGOT_PWD:
      "Masukkan alamat email Anda dan kami akan mengirimkan link untuk ganti password.",
    MSG_RESET_SENT:
      "Jika akun dengan email ini ada, Anda akan segera menerima link ganti password.",
    BUTTON_SEND_LINK: "Kirim Link",
    ERROR_PASSWORD_MISMATCH: "Password tidak cocok. Silakan coba lagi.",

    // Settings Page
    SETTINGS_TITLE: "Pengaturan Global",
    SETTINGS_SUBTITLE:
      "Kelola identitas perusahaan, tarif pajak dinamis, dan konfigurasi operasional inti.",
    SETTINGS_TAB_COMPANY: "Profil Perusahaan",
    SETTINGS_TAB_BANKS: "Rekening Bank",
    SETTINGS_TAB_PARAMETERS: "Parameter Sistem",

    // Settings Form Company Profile
    SETTINGS_SEC_COMPANY: "Spesifikasi Identitas Perusahaan",
    SETTINGS_LABEL_NPWP: "NPWP (Nomor Pajak)",
    SETTINGS_LABEL_LOGO_URL: "URL Logo Brand",
    SETTINGS_LABEL_HEADER_URL: "URL Gambar Header PDF",
    SETTINGS_LABEL_NPWP_ADDRESS: "Alamat NPWP",
    SETTINGS_LABEL_SHIPPING_ADDRESS: "Alamat Kantor Operasional / Pengiriman",
    SETTINGS_PLACEHOLDER_NPWP_ADDRESS:
      "Alamat resmi yang terdaftar di profil NPWP",
    SETTINGS_PLACEHOLDER_SHIPPING_ADDRESS: "Alamat kantor pusat operasional",
    SETTINGS_BUTTON_SAVE_COMPANY: "Simpan Spesifikasi",

    // Settings Bank Accounts
    SETTINGS_SEC_BANKS: "Rekening Bank Settlement Relasional",
    SETTINGS_ADD_BANK_ACC: "Tambah Rekening Bank",
    SETTINGS_EDIT_BANK_ACC: "Edit Rekening Bank",
    SETTINGS_LABEL_BANK_NAME: "Nama Bank",
    SETTINGS_LABEL_ACC_NUM: "Nomor Rekening",
    SETTINGS_LABEL_ACC_HOLDER: "Nama Pemilik Rekening",
    SETTINGS_LABEL_BRANCH: "Nama Cabang",
    SETTINGS_PLACEHOLDER_BANK_NAME:
      "misal: Bank Mandiri, BCA, Bank Rakyat Indonesia",
    SETTINGS_PLACEHOLDER_ACC_NUM: "misal: 111231002319",
    SETTINGS_PLACEHOLDER_ACC_HOLDER: "misal: PT Anugerah Buana Sriwijaya",
    SETTINGS_PLACEHOLDER_BRANCH: "misal: Letkol Iskandar",

    // Settings Parameters
    SETTINGS_SEC_PARAMS: "Parameter Operasional Dinamis",
    SETTINGS_BUTTON_SAVE_PARAMS: "Simpan Parameter",
    SETTINGS_ADD_PARAM: "Tambah Parameter",
    SETTINGS_LABEL_PARAM_KEY: "Key Parameter",
    SETTINGS_LABEL_PARAM_VALUE: "Nilai Parameter",
    SETTINGS_LABEL_PARAM_LABEL: "Label Parameter",
    SETTINGS_LABEL_PARAM_DESC: "Deskripsi Parameter",

    // Settings Email Routing
    SETTINGS_TAB_EMAILS: "Routing Email",
    SETTINGS_SEC_EMAILS: "Routing Email Otomatis (CC)",
    SETTINGS_BUTTON_SAVE_EMAILS: "Simpan Aturan Routing",

    // Verification Portal
    VERIFY_TITLE: "Verifikasi Dokumen",
    VERIFY_SUBTITLE: "Verifikasi keaslian dokumen Nexus dengan aman.",
    VERIFY_LABEL_QUOTATION_NUMBER: "Nomor Penawaran",
    VERIFY_PLACEHOLDER_QUOTATION_NUMBER: "misal: QTN/2026/001",
    VERIFY_BUTTON_VERIFY: "Verifikasi Dokumen",
    VERIFY_BUTTON_DOWNLOADING: "Mendownload...",
    VERIFY_SUCCESS_TITLE: "Keaslian Terkonfirmasi",
    VERIFY_SUCCESS_MSG:
      "Ini adalah dokumen asli yang diterbitkan oleh PT Anugerah Buana Sriwijaya.",
    VERIFY_MISMATCH_TITLE: "Keaslian Tidak Terkonfirmasi",
    VERIFY_MISMATCH_MSG:
      "Data dokumen ini mungkin telah dimodifikasi sejak kode QR ini dibuat.",
    VERIFY_ERROR_TITLE: "Verifikasi Gagal",
    VERIFY_ERROR_MSG:
      "Nomor penawaran tidak cocok dengan catatan ini. Silakan periksa dan coba lagi.",
    VERIFY_ERROR_RATE_LIMIT:
      "Terlalu banyak percobaan yang gagal. Silakan coba lagi dalam beberapa menit.",
    VERIFY_DOC_DETAILS: "Detail Dokumen",
    VERIFY_LABEL_COMPANY: "Perusahaan",
    VERIFY_LABEL_DATE: "Tanggal Terbit",
    VERIFY_LABEL_TOTAL: "Total",
    VERIFY_BUTTON_DOWNLOAD_OFFICIAL: "Download PDF Resmi",

    VERIFY_STATUS_HASH_MATCH: "Hash Cocok",
    VERIFY_STATUS_HASH_MISMATCH: "Hash Tidak Cocok",
    VERIFY_STATUS_HASH_MISMATCH_DESC:
      "Data dokumen mungkin telah dimodifikasi sejak kode QR ini dibuat.",
    VERIFY_STATUS_HASH_NOT_AVAILABLE:
      "Verifikasi hash tidak tersedia untuk dokumen ini",
    VERIFY_STATUS_HASH_VERIFIED: "Terverifikasi otomatis melalui hash QR",
    VERIFY_AUTO_VERIFYING: "Memverifikasi dokumen melalui kode QR...",
    VERIFY_LABEL_INVOICE_NUMBER: "Nomor Invoice",
    VERIFY_PLACEHOLDER_INVOICE_NUMBER: "misal: INV/2026/001",
  },
}

export type ContentKey = keyof typeof SITE_CONTENT.en
