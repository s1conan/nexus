  ...
  useEffect(() => {
    const handleAuth = async () => {
      ...
      if (userError) {
        ...
        router.push(`/auth/auth-code-error?error=${encodeURIComponent(userError.message)}`)
        return
      }
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-background p-4 text-center">
      <div className="space-y-4">
        ...
        <p className="text-muted-foreground">Please wait while we finalize your secure session.</p>
      </div>
    </div>
  )
}
