  ...
  try {
    ...
    if (action === "approve") {
      ...
      if (currentUser && profile.auth_id === currentUser.id) {
        console.warn(
          `Blocked self-revocation attempt for user: ${currentUser.email}`
        )
        return NextResponse.json(
      ...
      if (profileUpdateError) {
        console.error(
          "Profile Update Error during revocation:",
      ...
      console.log(
        `Profile unlinked for ${profileId}. Proceeding to background background cleanup.`
      )
