...
export default function DeliveryOrdersPage() {
  ...
  const [formData, setFormData] = useState(() => ({
    ...
    compartment_details: [] as { compartment_number: number; seal_number: string; quantity: number }[]
  }))
  ...
  const fetchData = useCallback(async (isInitial = false) => {
    ...
    try {
      ...
      if (debouncedSearchQuery) {
        const searchStr = constructMultiWordSearch(debouncedSearchQuery, ['do_number', 'company.name', 'product.sku', 'driver_info->>name', 'vehicle_number'])
  ...
  useEffect(() => {
    async function fetchRemainingSOQty() {
      ...
      let query = supabase.from("delivery_orders").select("quantity").eq("so_id", formData.so_id)
      // Exclude current DO when editing to avoid counting it twice
      if (editingItem) {
        query = query.neq("id", editingItem.id)
      }
      const { data } = await query
      const delivered = (data || []).reduce((sum: number, d: any) => sum + (d.quantity || 0), 0)
  ...
  useEffect(() => {
    for (const c of formData.compartment_details) {
      if (c.seal_number) sealNumberCache.current[c.compartment_number] = c.seal_number
    }
  ...
  const handleVehicleSelect = (vId: string, vehicle?: any) => {
    if (vehicle) {
      ...
      const compartments = (vehicle.compartments || []).sort((a: any, b: any) => a.number - b.number)
      setFormData(prev => ({
        ...prev,
        vehicle_id: vId,
  ...
  const handleOpenDialog = (item: any = null) => {
    if (item) {
      ...
      setFormData({
        ...
        driver_info: item.driver_info || { name: item.driver_name || "", phone: item.driver_phone || "" },
  ...
  const handleSave = async () => {
    ...
    if (!formData.quantity || formData.quantity <= 0) errors.push("Jumlah harus diisi.")
    ...
    if (remainingSOQty !== null && formData.quantity > remainingSOQty) {
      errors.push(`Jumlah melebihi sisa SO (${remainingSOQty.toLocaleString()} L).`)
    }
    ...
    try {
      ...
      if (editingItem) {
        const { error } = await supabase.from("delivery_orders").update(dbPayload).eq("id", editingItem.id)
        if (error) throw error
      } else {
      ...
      if (dbPayload.so_id) {
        const { data: allDOs } = await supabase.from("delivery_orders").select("quantity").eq("so_id", dbPayload.so_id)
        const totalDOQty = (allDOs || []).reduce((sum: number, doItem: any) => sum + (doItem.quantity || 0), 0)

        const { data: so } = await supabase.from("sales_orders").select("quantity, status").eq("id", dbPayload.so_id).single()
        if (so) {
          ...
          if (so.status !== newStatus) {
            await supabase.from("sales_orders").update({ status: newStatus }).eq("id", dbPayload.so_id)
          }
        }
      }
      ...
      if (editingItem) {
        notify.success(
          ...
          dict.MSG_SUCCESS_UPDATE_DESC.replace("%entity%", "delivery order").replace("%company%", `[${selectedCompanyInfo?.name || ""}]`),
          undefined,
          true
        )
      } else {
        notify.success(
          ...
          dict.MSG_SUCCESS_SAVE_DESC.replace("%entity%", "delivery order").replace("%company%", `[${selectedCompanyInfo?.name || ""}]`),
          undefined,
          true
        )
      }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      notify.deleted(
        ...
        dict.MSG_SUCCESS_DELETE_DESC.replace("%entity%", "delivery order").replace("%company%", `[${companyName}]`),
        undefined,
        true
      )
    } catch (err: any) {
  ...
  const handlePrint = async (o: any) => {
    ...
    const dataUri = await generateStandardDeliveryOrderPDF(
      companyInfo,
      o,
  ...
  const handleSendEmail = async (doc: any) => {
    try {
      ...
      const pdfDataUri = await generateStandardDeliveryOrderPDF(companyInfo, doRecord, { save: false, output: "datauri" })
      if (!pdfDataUri) throw new Error("Failed to generate PDF for attachment.");
      const attachments = [{ filename: `DO_${doc.title}.pdf`, content: (pdfDataUri as string).split(',')[1] }];

      // Build email HTML
      const customerName = doRecord.company?.details?.contact_person || doRecord.company?.name || "Valued Customer";
      const deliveryDate = doRecord.do_date ? new Date(doRecord.do_date).toLocaleDateString('en-GB', { day: '2-digit', month: 'long', year: 'numeric' }) : "-";
      const vehicleNumber = doRecord.vehicle?.license_number || doRecord.vehicle_number || "-";
      const driverName = doRecord.driver_info?.name || doRecord.driver_name || "-";
      const driverPhone = doRecord.driver_info?.phone || doRecord.driver_phone || "-";
      ...
      if (result.success) {
        notify.success(
          ...
          dict.MSG_EMAIL_SENT_SUCCESS_DESC?.replace("%data%", `[${doc.title}]`) || `Successfully sent email for [${doc.title}].`,
          undefined,
          true
        )
    ...
    <div className="page-container">
        ...
        <div className="flex items-center gap-2">
          ...
          <Dialog open={isOpen} onOpenChange={setIsOpen}>
            ...
            <DialogContent className="sm:max-w-5xl">
              <DialogHeader>
                <DialogTitle>
                  <Truck className="size-5 mr-2 inline-block" />{editingItem ? dict.BUTTON_EDIT + " DO" : dict.BUTTON_NEW_DO}
                </DialogTitle>
              </DialogHeader>
              <form onSubmit={(e) => { e.preventDefault(); handleSave(); }} id="do-form" className="p-6 flex-1 overflow-y-auto custom-scrollbar space-y-8">
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">

                  {/* Group 1: SO Information */}
                  <div className="lg:col-span-1 space-y-6">
                    <div className="space-y-4">
                      <h3 className="font-semibold text-base flex items-center gap-2 border-b pb-2"><Hash className="size-4 text-primary" /> {dict.LABEL_SO_INFORMATION}</h3>

                      <div className="grid gap-2">
                        <Label htmlFor="donum">{dict.LABEL_DO_NUMBER}</Label>
                        <Input id="donum" value={formData.do_number} onChange={e => setFormData({ ...formData, do_number: e.target.value })} disabled={editingItem && !hasPermission("delivery-order", "edit")} placeholder={dict.LABEL_AUTO_GENERATED} />
                      </div>

                      <div className="grid gap-2">
                        <Label className="text-primary font-bold">{dict.LABEL_SO_REQUIRED} *</Label>
                        <div className="flex gap-2">
                          <div className="flex-1">
                            <LiveSearch
                              key="so-search"
                              data={selectedPOInfo ? [{ ...selectedPOInfo, company_name: selectedCompanyInfo?.name }] : []}
                              fetchData={async (query) => {
                                let q = supabase.from("sales_orders").select("id, so_number, quantity, so_date, delivery_address, product_id, company_id, company:companies(id, name, details), product:products(id, sku, name)").in("status", ["Approved", "Partial"]).limit(8)
                                if (query) {
                                  const searchStr = constructMultiWordSearch(query, ['so_number', 'company.name'])
                              ...
                              visualColumns={[
                                { key: "so_number", header: dict.LABEL_SO_NUMBER, className: "w-1/2", primary: true },
                                { key: "company_name", header: dict.LABEL_COMPANY_NAME, className: "w-1/2" }
                              ]}
                      ...
                      <div className="grid gap-2">
                        ...
                        <Input value={isFromSO && selectedPOInfo?.so_date ? format(new Date(selectedPOInfo.so_date), "dd MMM yyyy") : ""} disabled className="bg-muted" placeholder={dict.PLACEHOLDER_FROM_SO} />
                      </div>
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div className="grid gap-2">
                          ...
                          <NumberInput value={selectedPOInfo?.quantity || 0} onChange={() => { }} disabled rightBadge="L" />
                        </div>
                        <div className="grid gap-2">
                          <div className="flex justify-between items-center">
                            <Label htmlFor="qty">{dict.LABEL_QTY_DELIVERED}</Label>
                            {remainingSOQty !== null && (
                              <span className={cn("text-[10px] font-bold px-2 rounded", remainingSOQty > 0 ? "bg-amber-100 text-amber-700" : "bg-red-100 text-red-700")}>
                      ...
                      <div className="grid gap-2">
                        <Label className="flex items-center gap-2"><MapPin className="size-4" /> {dict.LABEL_DELIVERY_ADDRESS}</Label>
                        {isFromSO && selectedCompanyInfo?.details?.addresses?.length > 0 ? (
                          <div className="bg-muted rounded-md px-3 py-2 border border-input">
                            {(() => {
                              const addrs = selectedCompanyInfo.details.addresses as { label: string; address: string }[]
                              const matched = addrs.find((a: any) => a.address === formData.delivery_address)
                              return matched ? (
                                <div className="flex flex-col">
                                  <span className="font-medium text-xs">{matched.label}</span>
                                  <span className="text-[10px] text-muted-foreground">{matched.address}</span>
                                </div>
                              ) : (
                                <span className="text-sm text-muted-foreground">{formData.delivery_address || dict.PLACEHOLDER_FROM_SO}</span>
                              )
                            })()}
                          </div>
                        ) : (
                          <Input value={formData.delivery_address} disabled={true} className="bg-muted h-12" placeholder={dict.PLACEHOLDER_FROM_SO} />
                        )}
                      </div>
                  ...
                  <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      <div className="space-y-4">
                        <h3 className="font-semibold text-base flex items-center gap-2 border-b pb-2"><Calendar className="size-4 text-primary" /> {dict.LABEL_DELIVERY_DETAILS}</h3>
                        ...
                        <div className="grid gap-2">
                          ...
                          <LiveSearch
                            data={selectedTransporterInfo ? [selectedTransporterInfo] : []}
                            fetchData={async (query) => {
                              let q = supabase.from("companies").select("id, name").contains('type', ['Transporter']).limit(8)
                            ...
                            searchColumns={["name"]}
                            visualColumns={[{ key: "name", header: dict.LABEL_TRANSPORTER, className: "w-full font-medium", primary: true }]}
                        ...
                        <div className="grid gap-2">
                          ...
                          <LiveSearch
                            data={formData.driver_info?.name ? [{ name: formData.driver_info.name, phone: formData.driver_info.phone }] : []}
                            fetchData={async (query) => {
                              let q = supabase.from("delivery_orders").select("driver_info").limit(50)
                              if (query) q = q.or(`driver_info->>name.ilike.%${query}%,driver_info->>phone.ilike.%${query}%`)
                              const { data } = await q
                              const uniqueDrivers = new Map();
                              (data || []).forEach((d: any) => {
                                if (d.driver_info && d.driver_info.name && !uniqueDrivers.has(d.driver_info.name)) {
                                  uniqueDrivers.set(d.driver_info.name, d.driver_info);
                                }
                              });
                              return Array.from(uniqueDrivers.values()).slice(0, 8);
                            }}
                            value={formData.driver_info?.name || ""}
                            onSelect={(val, item) => {
                              setFormData({ ...formData, driver_info: item || { name: val, phone: "" } })
                            }}
                            keyField="name"
                            displayField="name"
                            ...
                            visualColumns={[
                              { key: "name", header: "Name", className: "w-1/2 font-medium", primary: true },
                              { key: "phone", header: "Phone", className: "w-1/2 text-xs" }
                            ]}
                            placeholder={dict.PLACEHOLDER_SELECT_DRIVER}
                            emptyMessage={dict.MSG_TYPE_DRIVER_HINT}
                            allowCustomValue={true}
                            onCustomValue={(val) => setFormData({ ...formData, driver_info: { name: val, phone: formData.driver_info?.phone || "" } })}
                          />
                        </div>

                        <div className="grid gap-2">
                          <Label className="flex items-center gap-2"><Phone className="size-4" /> {dict.LABEL_DRIVER_PHONE}</Label>
                          <Input value={formData.driver_info?.phone || ""} onChange={e => setFormData({ ...formData, driver_info: { ...(formData.driver_info || {}), name: formData.driver_info?.name || "", phone: e.target.value } })} placeholder={dict.PLACEHOLDER_ENTER_DRIVER_PHONE} />
                        </div>
                        <div className="space-y-4">
                          <RichTextEditor
                      ...
                      <div className="space-y-4">
                        <h3 className="font-semibold text-base flex items-center gap-2 border-b pb-2"><Package className="size-4 text-primary" /> {dict.LABEL_LOGISTICS}</h3>

                        <div className="grid gap-2">
                          <div className="flex justify-between items-center">
                            <Label>{dict.LABEL_SUPPLIER_ORIGIN}</Label>
                            {availableStock !== null && (
                              <span className={cn("text-[10px] font-bold px-2 rounded", availableStock > 0 ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700")}>
                                {dict.LABEL_STOCK}: {availableStock.toLocaleString()}
                              </span>
                            )}
                          </div>
                          <LiveSearch
                            ...
                            fetchData={async (query) => {
                              ...
                              let stockQ = supabase
                                ...
                                .select("supplier_id, product_id, current_stock")
                                .eq("product_id", formData.product_id)
                                .gt("current_stock", 0)
                                .order("current_stock", { ascending: false })
                                .limit(8)
                              if (query) {
                                stockQ = stockQ.or(`supplier_id.in.(${query})`)
                              }
                              const { data: stockData } = await stockQ
                              if (!stockData?.length) return []
                              const supplierIds = stockData.map((s: any) => s.supplier_id)
                              const { data: suppliers } = await supabase
                                .from("companies")
                                .select("id, name")
                                .in("id", supplierIds)
                              const supplierMap = new Map((suppliers || []).map((c: any) => [c.id, c.name]))
                            ...
                            searchColumns={["name"]}
                            visualColumns={[
                              { key: "name", header: dict.LABEL_SUPPLIER_ORIGIN, className: "w-2/3", primary: true },
                              { key: "current_stock", header: "Stock", className: "w-1/3 font-mono text-right" }
                            ]}
                        ...
                        <div className="grid gap-2">
                          ...
                          <LiveSearch
                            ...
                            fetchData={async (query) => {
                              let q = supabase.from("vehicles").select("*").limit(8)
                        ...
                        <div className="space-y-3 h-[220px] overflow-y-auto">
                          {formData.compartment_details.length === 0 ? (
                            <div className="text-center p-2 text-muted-foreground border-2 border-dashed rounded-lg">{dict.MSG_SELECT_VEHICLE}</div>
                          ) : (
                            formData.compartment_details.map((comp, idx) => (
                              <Card key={idx} className="p-0 ring-0 shadow-none m-2">
                                <div className="flex items-center gap-3 mb-2">
                                  <span className="font-bold text-xl text-primary mt-4">#{comp.compartment_number}</span>
                                  <div className="grid gap-1 w-full">
                                    <Label className="text-[10px] uppercase">{dict.LABEL_SEAL_NUMBER}</Label>
                                    <Input size={1} value={comp.seal_number} onChange={e => {
                                  ...
                                  <div className="grid gap-1">
                                    ...
                                    <NumberInput value={comp.quantity} onChange={val => {
                                      const newDetails = [...formData.compartment_details]
              ...
              <DialogFooter className="p-5 border-t bg-muted/5 shrink-0">
                ...
                <Button onClick={() => handleSave()} disabled={isSaving || (editingItem ? !canEdit : !canInsert)}>{isSaving ? <ButtonLoader /> : <Save data-icon="inline-start" />} {dict.BUTTON_SAVE_DO}</Button>
              </DialogFooter>
            </DialogContent>
          </Dialog>
        </div>
      ...
      <div className="action-bar shrink-0 flex flex-col sm:flex-row gap-4 items-start sm:items-center">
        ...
        <Dialog open={isSortOpen} onOpenChange={setIsSortOpen}>
          <DialogTrigger asChild><Button variant="outline" size="sm" className="h-9"><ArrowUpDown className="size-4 mr-2" />{dict.LABEL_SORT}</Button></DialogTrigger>
            ...
            <div className="flex flex-col gap-4 p-5">
              {sortLevels.map((level, index) => (
                <div key={level.id} className="flex items-center gap-3">
                  ...
                  <Select value={level.column} onValueChange={(val) => setSortLevels(sortLevels.map(l => l.id === level.id ? { ...l, column: val } : l))}><SelectTrigger className="flex-1 h-9"><SelectValue /></SelectTrigger><SelectContent>
                    <SelectItem value="created_at">{dict.LABEL_DO_DATE}</SelectItem>
                    <SelectItem value="do_number">{dict.LABEL_DO_NUMBER}</SelectItem>
                    <SelectItem value="quantity">{dict.LABEL_QUANTITY}</SelectItem>
                    <SelectItem value="status">{dict.LABEL_STATUS}</SelectItem>
                  </SelectContent></Select>
                  <Button variant="outline" size="sm" className="h-9" onClick={() => setSortLevels(sortLevels.map(l => l.id === level.id ? { ...l, direction: l.direction === "asc" ? "desc" : "asc" } : l))}>{level.direction === "asc" ? <ArrowUpAZ className="size-4" /> : <ArrowDownZA className="size-4" />}</Button>
                  <Button variant="ghost" size="icon" className="size-9 text-destructive" disabled={sortLevels.length <= 1} onClick={() => { if (sortLevels.length > 1) setSortLevels(sortLevels.filter(l => l.id !== level.id)) }}><Trash2 className="size-4" /></Button>
                </div>
              ))}
              <Button variant="outline" size="sm" className="w-fit mt-2" onClick={() => setSortLevels([...sortLevels, { id: Math.random().toString(), column: "created_at", direction: "asc" }])}><Plus className="size-4 mr-2" />{dict.BUTTON_ADD_LEVEL}</Button>
            </div>
            <DialogFooter><Button variant="outline" onClick={() => setIsSortOpen(false)}>{dict.BUTTON_CANCEL}</Button><Button onClick={() => setIsSortOpen(false)}>{dict.LABEL_APPLY}</Button></DialogFooter>
          </DialogContent>
        </Dialog>
      </div>
              ...
              <TableRow key={o.id} className="group">
                ...
                <TableCell>
                  <div className="flex flex-col gap-0.5">
                    ...
                    <span className="text-[10px] text-muted-foreground">{o.driver_info?.name || o.driver_name || "-"}</span>
                  </div>
                </TableCell>
                <TableCell className="text-sm text-right font-mono">{o.quantity.toLocaleString()}</TableCell>
                <TableCell>
                  <div className={cn(
                  ...
                  <div className="flex items-center justify-end gap-1">
                    ...
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        <DropdownMenuSub>
                          <DropdownMenuSubTrigger><CheckCircle2 className="size-4 mr-2" /> {dict.LABEL_STATUS}</DropdownMenuSubTrigger>
                          <DropdownMenuPortal>
                            <DropdownMenuSubContent>
                              ...
                              <DropdownMenuItem onClick={() => updateStatus(o.id, 'Shipped')} className="text-blue-600 dark:text-blue-400 font-medium" disabled={!canEdit}>Shipped</DropdownMenuItem>
                              <DropdownMenuItem onClick={() => updateStatus(o.id, 'Delivered')} className="text-emerald-600 dark:text-emerald-400 font-medium" disabled={!canEdit}>Delivered</DropdownMenuItem>
                              <DropdownMenuItem onClick={() => updateStatus(o.id, 'Cancelled')} className="text-rose-600 dark:text-rose-400 font-medium" disabled={!canEdit}>Cancelled</DropdownMenuItem>
                            </DropdownMenuSubContent>
                          </DropdownMenuPortal>
                        </DropdownMenuSub>
                        {canDelete && (
