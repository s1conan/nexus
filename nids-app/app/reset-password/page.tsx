...
function SetupPasswordForm() {
  ...
  if (success) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-muted/40 p-4">
        <Card className="w-full max-w-md text-center animate-in zoom-in-95 duration-500 shadow-xl border-emerald-500/20">
          <CardHeader>
  ...
  return (
    <div className="login-background">
      <div className="login-card relative animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="p-8 pt-4">
          {criticalError ? (
            <div className="flex flex-col gap-6">
              <div className="p-4 text-sm text-destructive bg-destructive/10 border border-destructive/20 rounded-lg text-center font-medium">
                {criticalError}
              </div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
              ...
              <div className="flex flex-col gap-2">
                <Label htmlFor="confirmPassword">{dict.LABEL_CONFIRM_PASSWORD}</Label>
                <div className="relative group">
                  <Lock className="absolute left-3 top-3 size-4 text-muted-foreground transition-colors group-focus-within:text-primary" />
                  <Input
                    id="confirmPassword"
              ...
              <Button type="submit" className="w-full h-11 font-semibold tracking-wide mt-2" disabled={loading}>
                {loading ? (
                  <ButtonLoader />
                ) : (
                  dict.BUTTON_SET_NEW_PWD
                )}
              </Button>
            </form>
          )}

          <div className="mt-8 text-center text-xs text-muted-foreground font-medium tracking-widest opacity-60">
            {dict.COPYRIGHT}
          </div>
        </div>
      </div>
